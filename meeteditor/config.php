...
const DEFAULT_SETTINGS = [
    ...
    // Зменшення щойно ЗГЕНЕРОВАНОГО фото під розмір плитки Meet — пропорційно
    // (cover), лише downscale (без апскейлу). Робиться у воркері ДО деградації:
    // спершу низька роздільність камери, потім поганий бітрейт кодека.
    'gen_resize'         => '1',        // '1' = зменшувати, '0'/'' = лишати як є
    'gen_resize_w'       => '139',      // цільова ширина плитки, px
    'gen_resize_h'       => '185',      // цільова висота плитки, px
    // Авто-деградація щойно ЗГЕНЕРОВАНИХ картинок — бейкається у байти прямо у
    // воркері (gen_worker.php), одразу після відповіді моделі. Незалежно від cam_*
    // (ті — для рендера/скрінів). Кожна картинка отримує ВИПАДКОВУ силу в межах
    // [min,max]. Дефолт: чистий кодек (gd-jpeg), 60..100%.
    'gen_degrade'        => '1',        // '1' = вмикати, '0'/'' = вимкнути
    'gen_degrade_method' => 'gd-jpeg',  // server-метод бейку: gd | gd-jpeg
    'gen_degrade_min'    => '60',       // нижня межа випадкової сили (%)
    'gen_degrade_max'    => '100',      // верхня межа випадкової сили (%)
];
