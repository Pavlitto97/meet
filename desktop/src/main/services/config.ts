 ...
 * аватарні кольори Meet). Колір — детермінована функція ІМЕНІ учасника
 * (render.ts::nameColor, FNV-1a hash по палітрі): те саме ім'я → завжди той
 * самий колір, однаково на «початку» і «кінці» та при будь-якому порядку.
