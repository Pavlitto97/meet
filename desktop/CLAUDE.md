...
- **Видалення учасника** (`DELETE /api/participants/<id>`): user_added —
  ...
  `POST .../restore`, список — `GET /api/participants?deleted=1`. Рендер **ховає
  всі появи** видаленого слота CSS-ом (`headInject` у `render.ts`): плитку сітки і
  рядок панелі «Люди» (за `data-participant-id`), бейдж People (`Qw4c9e`) — за
  унікальним `src` буквеного файла; лічильники «Люди» (`fs3avc`/`MKVSQd`) і лейбл
  «Ще N осіб» зменшуються. Кружечки-прев'ю «Ще N осіб» (`qg7mD`, рівно 2 слоти) НЕ
  прив'язані до власника — **переназначаються** першим N активним «іншим»
  (302/306/307), тож видалення одного промотує наступного у вільний слот (а не
  лишає 1 кружечок). Тобто видалений учасник зникає зі скрінів «початок»/«кінець»,
  а не показує рідну плитку. «Інших» рахуємо за ШАБЛОНОМ (слоти без файла ролі
  `tile`), не за БД-прапором `skipped` (у міграованих групах він = 0).
