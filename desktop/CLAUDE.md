...
- **Шаблон** — збережена сторінка Meet **mqy-kiph-fci** (укр. локаль, 24-год час
  ...
  актуального імені, колір — детермінована функція ІМЕНІ (`render.ts::nameColor`,
  FNV-1a hash по палітрі `LETTER_COLORS`): те саме ім'я → завжди той самий колір,
  однаково на start/end і при будь-якому порядку/групі. **Sandro (devices/316)
  закріплений**: завжди `#8d6e63` з S. Head-inject пінить `c-wiz.SSPGKf` до 2560×1271 і вмикає
