...
- **Шаблон** — збережена сторінка Meet **mqy-kiph-fci** (укр. локаль, 24-год час
  ...
  актуального імені, колір — з палітри `LETTER_COLORS` (20 ЕТАЛОННИХ аватарних
  кольорів Meet) через `render.ts::assignAvatarColors`: бажаний колір — FNV-1a hash
  імені (`nameColorIndex`), але в межах ОДНІЄЇ групи кольори **дедуплікуються** (зайнятий
  → зсув до наступного вільного). Тож у групі до 20 не-Sandro усі кольори унікальні, а
  start/end і будь-який порядок (drag-and-drop) фарбують однаково (дедуп за відсортованим
  іменем, не за позицією). **Sandro (devices/316) закріплений**: завжди `#8d6e63` з S
  (еталонний коричневий `#8c6d63` виключено з пулу, щоб решта не зливалась із Sandro).
  Head-inject пінить `c-wiz.SSPGKf` до 2560×1271 і вмикає
...
- **Групи** (`services/groups.ts`, таблиця `groups`): кілька іменованих складів
  ...
  **Активна група** (settings `active_group_id`, самолікується) — її учасників
  ...
  видалити не можна.
- **Розкладка по слотах ЗА ПОЗИЦІЄЮ (render.ts):** рендер НЕ прив'язує учасника
  до його `device_id`, а **пакує активних учасників у слоти шаблону за порядком
  `position`**. Слоти (канонічний порядок = `Object.keys(PARTICIPANT_ASSETS)`):
  Sandro (316) **закріплений** у своєму слоті-презентері; решта (за position)
  заповнюють `[295..301]` (7 плиток-фото), далі `[302,306,307]` («Ще N осіб» — лише
  рядок+кружечок, без фото). Тож сортування у редакторі (drag-and-drop за ручку
  в колонці № або стрілки ↑↓) РУХАЄ фото в рендері:
  перших 7 не-Sandro → плитки-фото. У кожному слоті підміняються фото/літера ТА
  ім'я слота (двофазно через NUL-плейсхолдери, щоб уникнути колізій імен). «Інших»
  визначаємо за ШАБЛОНОМ (слоти без файла ролі `tile`), не за `skipped`. Запит
  учасників фільтрує `deleted = 0 AND skipped = 0` — **пропущені (skipped=1) у рендер
  НЕ йдуть** (ні плитка, ні рядок «Люди», ні лічильник): «Пропуск» = «не показувати».
  Дефолти 302/306/307 НЕ pre-skipped — пропуск суто користувацький.
- **Видалення учасника** (`DELETE /api/participants/<id>`): user_added —
  назавжди; дефолтний слот — м'яко (`deleted=1`, правки скинуто), відновлення —
  `POST .../restore`, список — `GET /api/participants?deleted=1`. У рендері видалення
  = **УЩІЛЬНЕННЯ**: видалений просто відсутній серед активних, решта зсуваються без
  дірок, а зайві (незаповнені) слоти ховаються CSS-ом (`headInject`): плитка сітки і
  рядок панелі «Люди» за `data-participant-id`, кружечок (`qg7mD`)/бейдж (`Qw4c9e`)
  за унікальним `src`. Лічильники «Люди» (`fs3avc`/`MKVSQd`) = зайняті слоти + 1;
  «Ще N осіб» N = усі не-Sandro поза 7 плитками-фото. Тобто видалений зникає зі скрінів
  «початок»/«кінець», а не показує рідну плитку.
  ⚠️ Список РЯДКІВ панелі «Люди» — у фіксованому DOM-порядку шаблону (кожен рядок =
  призначений слоту учасник), тож порядок рядків панелі ≠ position (плитки-фото —
  правильні).
- **Понад 10 учасників (надмір):** не-Sandro понад 10 слотів (ранг ≥ 10) шаблонних
  плиток/рядків не мають — рендер **КЛОНУЄ рядок панелі «Люди»** (`extractPanelRow`
  device 295 + `findDivClose`; підміна імені/аватарки/`data-participant-id`) і вставляє
  в кінець `div[role="list"].AE8xFb`. Лічильник «Ще N осіб» рахує і їх (без обмеження 3).
  Фото-плиток усе одно 7 — надмір видно лише у списку «Люди» та у числі «Ще N».
...
- `renderer/` — Vue 3 + Vite SPA (hash-роутинг), білд у `renderer/dist`. Вкладки
  ...
  зображеннями Оригінал/Початок/Кінець). ⚠️ Вкладка «Групи» (`/admin/groups`)
  **авто-редіректить на детальну АКТИВНОЇ групи**; повний список — лише `?list=1`
  (кнопка «← Групи» у детальній). Тести беруть список через `helpers.gotoGroupList`.
  Далі — Генерації (фільтри група/учасник/статус,
